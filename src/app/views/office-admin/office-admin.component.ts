...
export class OfficeAdminComponent implements OnInit {
  ...
  role: any;
  officeadminList: any;
  listTitle:string = "Office Admins List";
  listTableHeads = ['No', 'Name', 'Email'];
  ...
  getOfficeAdmin() {
    ...
    this.apiProvider.getOfficeAdmin().subscribe(response => {
      if (response['status'] == '1') {
        ...
        this.listTitle = `Office Admins List (${this.officeadminList.length})`;
      }
    })
  }
  ...
  updateSearch(event){
    this.searchOpt.title = event.name;
    this.officeadminList = this.filterSearch();
  }
}
